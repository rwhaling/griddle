  ...
  return {
    ...
    async transform(_code, id) {
      ...
      const output = await build({
        ...
        // griddle local delta (see vendor/strudel/README.md): the inner build
        // sees no config file, so thread the host config's resolve (aliases)
        // through — the worklet entry may live outside this project root
